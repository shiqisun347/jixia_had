...
新建房间只接受 `side_size = 4` 的已启用规则。Web 只展示 4v4 选项，Core
在创建事务内独立校验，避免旧客户端或直接 API 调用绕过。数据库约束、规则
解析器和 MatchActor 保留对历史 1v1 至 5v5 快照的读取与恢复能力；这属于
兼容边界，不代表新产品范围，也不触发历史数据改写。
...
- 每个模型有独立上限，且不得超过全局上限。
- 调度器按比赛轮转公平分配容量，避免一场 4v4 自由辩论占满全部槽位。
...
创建或修改赛制当前配置时生成变更的主持文本和音频，记录文本哈希、voice、rate、模型版本和文件状态。启用规则前必须全部生成成功。比赛过程只读取房间冻结的本地预生成资产，不临时调用 TTS，也不播报动态辩手姓名。
...
| `rules` / `rule_stages` / `stage_actions` | 赛制当前配置、线性阶段、动作、启用与归档状态 |
| `stage_prompt_templates` / `agent_prompt_overrides` | 规则阶段通用 Prompt 与规则 Agent 的最小完整文本覆盖 |
| `rule_judge_configs` | 规则独立的裁判开关、模型、完整 Prompt 与排行榜开关 |
...
创建房间事务不接收房主预选席位，也不允许关闭 Agent 全席位预填。普通房主写为无席位的 `DEBATER`，纯 Agent 房管理员写为 `ORGANIZER`；从所选规则的启用 Agent 池随机且不重复地填满八席，并在同一事务写入完整规则、Prompt、Agent、模型、音色和裁判快照。数量不足时整笔创建回滚。真人选席仅把目标 Agent 席位改为 HUMAN，`configured_agent_profile_id` 保留，以便换席、切换观众或离开时恢复。纯 Agent 房在加入、身份切换和选席三个服务边界都拒绝人类辩手，只允许观众。开赛事务除校验席位完整和真人检测外，还必须拒绝任何没有真人席位的活动 `DEBATER`。辩手切换观众与观众加入共享同一个 `CapacityGuard` 和全局计数，避免绕过 10 人上限。

正式实验排表允许 Agent 位于任意席位。控制权限不能依赖一辩为人类：Core 按每方席位号最小的
真人确定两名控制者，正方控制者写入 `rooms.organizer_user_id`；训练房仍由首次进入的预约参与者
组织。HTTP、WebSocket、开赛、终止和 Web 展示必须共用该判定。等待中的实验房直接终止时，
同一事务把关联 attempt 标记为 `TERMINATED`、排表标记为 `INCOMPLETE`，避免活动状态残留。

普通用户最多参与一个未结束房间，普通房主最多一个未关闭房间；管理员创建的全 Agent 房间允许多房间管理，但仍受全局 5 场活动容量限制。规则、Agent、模型、音色和裁判配置允许按各自生命周期修改，修改只影响新房间；活动和历史比赛读取自身快照。删除仍受引用保护，API Key 轮换可原地执行。
...
模型、Agent、音色、题库和赛制编辑必须在事务内锁定目标行、重新校验唯一性和引用状态并写审计。运行配置修改只影响之后创建的房间；已创建房间的冻结快照不可改写。音色供应商标识、rate 或校准变化后，试听资产失效并同步规则 Agent 池；主持音色变化继续遵守主持资产重新生成规则。
...
4. 赛制规则快照、主持音频生成、房间角色、席位、设备检测和公开大厅；旧赛制版本仅保留历史读取兼容。
